...
class ProtoNetEmbedding(nn.Module):
    def __init__(self, x_dim=3, h_dim=64, z_dim=64, retain_last_activation=True, out_channels = 1600):
...
class ProtoNetEmbedding_N(nn.Module):
    def __init__(self, x_dim=3, h_dim=64, z_dim=64, retain_last_activation=True, out_channels = 1600):
        super(ProtoNetEmbedding_N, self).__init__()
        self.out_channels = out_channels
        self.normalize = nn.LayerNorm(self.out_channels, elementwise_affine=False)
        self.encoder = nn.Sequential(
          ConvBlock(x_dim, h_dim),
          ConvBlock(h_dim, h_dim),
          ConvBlock(h_dim, h_dim),
    ...
    def forward(self, x):
        ...
        #print('size:', x.size())
        x = self.normalize(x)
        #print('norm', torch.norm(x[0]))
        return x


class ProtoNetEmbedding_Opt(nn.Module):

    def __init__(self, config, protonet):
        super(ProtoNetEmbedding_Opt, self).__init__()
        self.config = config
        self.base = protonet
        self.out_channels = self.base.out_channels
        self.OptNet = OptNet(config)
