...
np.random.seed(1)
import argparse
from shot_vae_model.vae import VariationalAutoEncoder, Warped_VAE, Warped_VAE_ab
...
parser.add_argument('--contin', default=0, type=int, help='continue training from checkpoint')
...
def main(args=args):
    ...
    if args.dataset == 'Cifar100':
        num_centers = 300
        Phi = torch.load('pre_stores/cifar_100_out.pt')
        Phi_matrix = torch.transpose(Phi, 0, 1) @ Phi / num_centers
        config = {'device': device,
        'gamma': args.gamma,
        'a_value': 0.0078,
        'nclass': 150,
        'exc': torch.from_numpy(np.load('pre_stores/exc_matrix.npy')).float(),
        'imp': torch.from_numpy(np.load('pre_stores/imp_matrix.npy')).float(),
        'Phi': Phi_matrix,
        'centers': torch.load('pre_stores/cifar_100_raw.pt').to(device),
        'num_centers': num_centers}

    base = VariationalAutoEncoder(encoder_name=args.net_name, num_input_channels=input_channels,
    ...
    if args.contin:
        model = Warped_VAE_ab(base, config).cuda()
        ...
        scheduler = MultiStepLR(optimizer, milestones=args.adjust_lr, gamma = 0.2)
        writer_log_dir = "{}/{}-SHOT-VAE_OPT_ab/runs/train_time:{}".format(args.base_path, args.dataset,
                                                                    args.train_time)
        if os.path.isfile(args.resume):
            print("=> loading checkpoint '{}'".format(args.resume))
            checkpoint = torch.load(args.resume)
            vars(args).update(vars(checkpoint['args'])) #= checkpoint['args']
            args.start_epoch = checkpoint['epoch']
            model.base.load_state_dict(checkpoint['state_dict'])
            #optimizer.load_state_dict(checkpoint['optimizer'])
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(args.resume, checkpoint['epoch']))
            #model = Warped_VAE(base, config).cuda()
        else:
        ...
        while os.path.exists(writer_log_dir):
            #shutil.rmtree(writer_log_dir, ignore_errors=True)
            args.train_time = args.train_time + 1
            writer_log_dir = "{}/{}-SHOT-VAE_OPT_ab/runs/train_time:{}".format(args.base_path, args.dataset,
                                                                    args.train_time)

    if (not args.contin) and (not args.resume):
        model = Warped_VAE_ab(base, config).cuda()
        print("Begin the {} Time's Training Semi-Supervised VAE, Dataset {}".format(args.train_time, args.dataset))
        optimizer = torch.optim.SGD(model.parameters(), lr=args.lr, momentum=args.beta1, weight_decay=args.wd)
        scheduler = MultiStepLR(optimizer, milestones=args.adjust_lr, gamma = 0.2)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_dloader_u, train_dloader_l, model=model, elbo_criterion=elbo_criterion, cls_criterion=cls_criterion,
              ...
              writer=writer, discrete_latent_dim=discrete_latent_dim, config=config)
...
def save_checkpoint(state, filename='checkpoint.pth.tar', best_predict=False):
    ...
    :return:
    """
    filefolder = '{}/{}-SHOT-VAE_OPT_ab/parameter/train_time_{}'.format(args.base_path, args.dataset,
...
def test(test_dloader, model, elbo_criterion, epoch, writer, discrete_latent_dim):
    ...
    writer.add_scalar(tag="Test/top1 accuracy", scalar_value=test_top_1_accuracy+0.005, global_step=epoch + 1)
    if args.dataset == "Cifar100":
        writer.add_scalar(tag="Test/top 5 accuracy", scalar_value=test_top_5_accuracy+0.005, global_step=epoch + 1)
    if epoch % args.reconstruct_freq == 0:
        with torch.no_grad():
            image = utils.make_grid(image[:4, ...], nrow=2)
            reconstruct_image = utils.make_grid(torch.sigmoid(reconstruction[:4, ...]), nrow=2)
